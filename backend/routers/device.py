...
def register_device(request: DeviceCreate, db: Session = Depends(get_db)):
    existing_device = (
        db.query(Device).filter(Device.serial_number == request.serial_number).first()
    )
...
def assign_firmware(request: AssignFirmwareRequest, db: Session = Depends(get_db)):
    device = (
        db.query(Device).filter(Device.serial_number == request.serial_number).first()
    )
    ...
    firmware = (
        db.query(Firmware).filter(Firmware.version == request.firmware_version).first()
    )
...
def update_device_status(request: DeviceStatusRequest, db: Session = Depends(get_db)):
    device = (
        db.query(Device).filter(Device.serial_number == request.serial_number).first()
    )
    ...
    allowed_status = ["Pending", "Updating", "Updated", "Failed"]
...
def search_device_by_serial(serial_number: str, db: Session = Depends(get_db)):
...
def get_devices_by_status(status: str, db: Session = Depends(get_db)):
    devices = db.query(Device).filter(Device.status == status).all()

    return devices


@router.get("/firmware/{version}")
def get_devices_by_firmware(version: str, db: Session = Depends(get_db)):
    devices = db.query(Device).filter(Device.firmware_version == version).all()
...
def update_device(device_id: int, request: DeviceUpdate, db: Session = Depends(get_db)):
    device = db.query(Device).filter(Device.id == device_id).first()
