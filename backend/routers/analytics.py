...
def firmware_distribution(db: Session = Depends(get_db)):
    data = (
        db.query(Device.firmware_version, func.count(Device.id))
        .group_by(Device.firmware_version)
        .all()
    )

    return [{"version": version, "devices": count} for version, count in data]
